...
We advance two primary hypotheses. The **Visuomotor Collapse Hypothesis** predicts that power in the 0.5–3.0 Hz tracking band will drop following probe presentation on difficult trials, suggesting that perceptual challenge temporarily withdraws attentional resources from the motor-control loop and reduces active correction around the target force. The **Tremor Surging Hypothesis** predicts that, under high physical strain, power in the 8.0–12.0 Hz tremor band will increase as perceptual difficulty rises and will predict subsequent choice failures, supporting the interpretation that high-frequency force instability is an online biophysical signature of cognitive-motor strain rather than peripheral motor noise.
...
**Target normalization.** Raw force values were divided by each participant's MVC-derived target force for that trial (5% MVC or 40% MVC), expressing deviations as a proportion of the instructed level. This normalization ensures that spectral features reflect tracking quality relative to the intended effort level rather than between-subject differences in absolute grip strength.
...
- *Visuomotor tracking band* (0.5–3.0 Hz): average log-power reflecting slow corrective adjustments to keep force near the target. A reduction in this band after probe onset may indicate withdrawal of attentional resources from the motor-control loop.
...
Because low- and high-grip trials reflect different motor-control regimes, we repeated the analysis separately within each grip stratum (@fig-direction-flip). Under high grip, each +1 SD increase in tremor power was associated with lower accuracy (OR = `{r} txt_strat_high_or`, *p* = `{r} txt_strat_high_p`), consistent with the Tremor Surging Hypothesis in direction if not in significance. Under low grip, the association was similar in sign and magnitude (OR = `{r} txt_strat_low_or`, *p* = `{r} txt_strat_low_p`), indicating that low-effort spectral noise does not show the predicted positive coupling with accuracy. The Visuomotor Collapse Hypothesis was not supported in the primary high-grip model: tracking-band power did not predict accuracy (OR = `{r} txt_track_high_or`, *p* = `{r} txt_track_high_p`). @fig-tremor-lift shows the tremor × grip interaction on marginal accuracy after adjusting for modality and difficulty.

Subject-wise cross-validation assessed whether grip-force features generalize beyond task difficulty. A random-forest classifier using stimulus difficulty and modality alone achieved out-of-sample AUC = `{r} txt_auc_base`; adding DSP features raised AUC to `{r} txt_auc_dsp` (Δ = `{r} txt_auc_delta`). The increment was small but consistent, indicating that continuous force dynamics carry incremental—not dominant—predictive information about trial-level accuracy ([Supplementary Figure S7](supplementary.qmd#fig-supp-ml-roc)).
...
Together, the results offer qualified support for the Tremor Surging Hypothesis under high physical load: tremor-band power trended in the predicted negative direction, although trial-level coefficients did not reach conventional significance after controlling for difficulty and subject structure. The Visuomotor Collapse Hypothesis was not supported. Cross-validated prediction confirmed a modest incremental gain from force features beyond nominal task labels (ΔAUC = `{r} txt_auc_delta`), consistent with a subtle rather than dominant motor signature. Exploratory LC analyses yielded a non-significant directional trend warranting higher-powered replication.
...
![Stratified tremor–accuracy associations under high (40% MVC) versus low (5% MVC) grip. Left: population-level predicted accuracy across within-stratum tremor *z*-scores from mixed-effects models with modality and difficulty covariates (predictions averaged across auditory and visual tasks and all stimulus levels); points show binned observed accuracy. Right: odds-ratio forest plot for tremor power.](figures/hero_direction_flip.png){#fig-direction-flip width=95% fig-column="span"}
