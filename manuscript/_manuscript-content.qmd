...
if (file.exists(trial_path)) {
  ...
  is_hi  <- tolower(as.character(df$is_high_grip)) %in% c("true", "1")
  ...
  n_excl <- sum(abs(df_clean$pow_8_12hz_z) > 3.5, na.rm = TRUE)
  ...
  txt_age_sd     <- sprintf("%.1f", age_sd)
  txt_pct_female <- sprintf("%.0f", pct_female)
  txt_n_spectral_excl  <- "~30"
  txt_pct_spectral_excl <- "~0.4"
}
```
...
txt_tremor_diff_df   <- as.integer(lrt_tremor_diff$Df[2] - lrt_tremor_diff$Df[1])
...
txt_grip_int_df   <- as.integer(lrt_grip_int$Df[2] - lrt_grip_int$Df[1])
...
After behavioral exclusions and spectral quality screening, the primary analysis included `{r} txt_n_high_subj` participants contributing `{r} txt_n_high_trials` high-grip trials (40% MVC; accuracy = `{r} txt_high_acc`%) and `{r} txt_n_low_trials` low-grip trials (5% MVC; accuracy = `{r} txt_low_acc`%). Participants maintained high-grip targets with good fidelity (mean compliance = `{r} txt_compliance`× instructed force; [Supplementary Figure S4](supplementary.qmd#fig-supp-compliance)).

Physical load compressed perceptual sensitivity. @fig-psychometric shows that *P*("Different") rose monotonically with stimulus level in both modalities, but high-grip trials produced lower sensitivity at intermediate levels (Δ2–Δ3), consistent with a dual-task cost concentrated where evidence is weakest. At the subject level, accuracy under high grip was lower than under low grip for `{r} txt_pct_hindered`% of participants (mean load cost = `{r} txt_load_cost_mean` percentage points, *SD* = `{r} txt_load_cost_sd`). The load-related accuracy drop was not accompanied by a systematic speed–accuracy trade-off ([Supplementary Figure S6](supplementary.qmd#fig-supp-sat)).

Grand-average Welch PSDs differed markedly between grip conditions (@fig-psd-ribbon). Under high grip, power in the 8–12 Hz tremor band was suppressed relative to low grip, whereas the 0.5–3 Hz tracking band remained elevated—reflecting active visuomotor correction around the 40% MVC target. Trial-level distributions showed that incorrect responses tended to cluster toward higher tremor power under high effort ([Supplementary Figure S3](supplementary.qmd#fig-supp-spectral-fingerprint)).

## Inferential models and hypothesis tests

We first tested whether log band-power features improved a baseline mixed-effects logistic model of perceptual accuracy in the high-grip arm. Adding tracking- and tremor-band power did not significantly improve model fit (likelihood-ratio test: χ²(2) = `{r} txt_lrt_chisq`, *p* = `{r} txt_lrt_p`). In the augmented model, higher tremor power showed a numerically negative association with accuracy (OR = `{r} txt_tremor_high_or`, *p* = `{r} txt_tremor_high_p`), whereas tracking power was essentially flat (OR = `{r} txt_track_high_or`, *p* = `{r} txt_track_high_p`). Stimulus difficulty and modality remained strong predictors in both models (@tbl-glmer-high; [Supplementary Table S3](supplementary.qmd#tbl-supp-glmer)).

We next examined whether tremor-accuracy associations differed across grip conditions (@fig-direction-flip). Stratified models showed numerically negative tremor coefficients in both strata: high grip OR = `{r} txt_strat_high_or`, *p* = `{r} txt_strat_high_p`; low grip OR = `{r} txt_strat_low_or`, *p* = `{r} txt_strat_low_p`. A pooled model formally testing the tremor × grip interaction did not support differential effects (χ²(`{r} txt_grip_int_df`) = `{r} txt_grip_int_chisq`, *p* = `{r} txt_grip_int_p`), indicating that the negative tremor-accuracy direction was not specific to high effort. We additionally tested the Tremor Instability Hypothesis more directly by asking whether tremor power scaled with perceptual difficulty under high grip; the tremor × difficulty interaction was not significant (LRT χ²(`{r} txt_tremor_diff_df`) = `{r} txt_tremor_diff_chisq`, *p* = `{r} txt_tremor_diff_p`), indicating no difficulty-contingent tremor elevation within the high-grip arm. There was no support for the Visuomotor Tracking Hypothesis: tracking-band power did not predict accuracy in the primary high-grip model (OR = `{r} txt_track_high_or`, *p* = `{r} txt_track_high_p`). @fig-tremor-lift illustrates marginal accuracy by tremor quantile, split by grip.

Subject-wise cross-validation assessed out-of-sample generalization of grip-force features. A random-forest classifier using stimulus difficulty and modality alone achieved AUC = `{r} txt_auc_base`; adding DSP features raised AUC to `{r} txt_auc_dsp` (Δ = `{r} txt_auc_delta`). This increment is small and should be interpreted cautiously: baseline and DSP models were evaluated on slightly different trial counts due to feature availability, no bootstrap confidence interval was computed for ΔAUC, and no permutation test was conducted. At most, the directional result indicates minimal incremental information from force dynamics above task labels, not a validated predictive gain ([Supplementary Figure S7](supplementary.qmd#fig-supp-ml-roc)).

In an exploratory analysis linking MRI-derived locus coeruleus (LC) microstructure to load-related motor reactivity (*N* = `{r} txt_lc_n`), LC fICVF showed a directional trend toward larger high-minus-low grip increases in tremor power among participants with lower integrity (*r* = `{r} txt_lc_tremor_r`, 95% bootstrap CI [`{r} txt_lc_tremor_lo`, `{r} txt_lc_tremor_hi`], interval including zero). This pattern is consistent with, but does not confirm, a neuromodulatory boundary condition on motor reactivity under load. The association between LC fICVF and subject-level load cost was weaker and not reliable (*r* = `{r} txt_lc_cost_r`, 95% CI [`{r} txt_lc_cost_lo`, `{r} txt_lc_cost_hi`]; [Supplementary Figure S8](supplementary.qmd#fig-supp-lc-behavior)).

In summary, force features provided weak, non-significant inferential evidence: adding band-power to the baseline model did not significantly improve fit, tremor-band power showed a small same-direction negative association across both grip strata with no significant grip interaction or difficulty interaction, and tracking-band power was near zero. Cross-validated models confirmed only minimal incremental predictive value beyond task difficulty (ΔAUC = `{r} txt_auc_delta`), with substantial caveats about model comparability. Exploratory LC microstructure analyses yielded non-significant directional trends warranting replication in a larger sample.
...
![Stratified tremor–accuracy associations under high (40% MVC) and low (5% MVC) grip. Both conditions show small negative associations of similar sign and magnitude; a formal tremor × grip interaction was not significant (χ²(1) = `{r} txt_grip_int_chisq`, *p* = `{r} txt_grip_int_p`). Left: population-level predicted accuracy from stratum-specific mixed-effects models (averaged across modality and all difficulty levels); points show binned observed accuracy. Right: odds-ratio forest plot for tremor-band power.](figures/hero_direction_flip.png){#fig-direction-flip width=95% fig-column="span"}

![Marginal accuracy as a function of tremor-band power, split by grip effort. Lines show population-level fixed-effect predictions from a tremor × grip interaction model with modality and orthogonal difficulty contrasts (all stimulus levels); points show binned observed accuracy ± 95% CI (bins with *n* ≥ 25).](figures/hero_tremor_accuracy_lift.png){#fig-tremor-lift width=85%}

```{r}
#| label: tbl-glmer-high
#| echo: false
#| tbl-cap: "Mixed-effects logistic regression of perceptual accuracy — high-grip (40% MVC) trials. Odds ratios (OR) with 95% Wald confidence intervals; random intercepts for participant."
...
knitr::kable(fx_high, align = c("l", "r", "r"))
```

# Discussion {#sec-discussion}

This study developed and evaluated a trial-level spectral pipeline for extracting grip-force dynamics as online behavioral signals of cognitive-motor strain in older adults. The primary inferential results were null: adding visuomotor tracking and tremor band features to a baseline psychometric model did not significantly improve prediction of perceptual accuracy under high physical load, and neither hypothesis received direct statistical support. Tremor-band power showed a small directionally negative association with accuracy, but this association was non-significant, did not interact with grip level, and did not scale with perceptual difficulty. Tracking-band power was near zero. Cross-validated models confirmed only minimal incremental predictive gain beyond task difficulty labels, with caveats about model comparability. These results are broadly consistent with a small, non-specific motor instability effect rather than a load-stratified biophysical marker of perceptual strain.

The absence of a significant tremor effect is informative in its own right. The Resource Competition account predicts that high physical load should degrade perceptual performance by diverting shared executive resources from evidence accumulation to motor maintenance. We observed a behavioral load cost—accuracy was lower under high grip for the majority of participants—consistent with this general prediction. However, the continuous force dynamics did not significantly track this cost at the trial level once task difficulty was controlled. This dissociation suggests that the behavioral dual-task cost may arise primarily from difficulty × effort interactions captured by the nominal experimental conditions, rather than from moment-to-moment fluctuations in the spectral structure of force output. Alternatively, the present epoch design—averaging spectral power over a multi-second window aligned to grip-cue onset—may obscure the sub-epoch dynamics most relevant to decision-making, such as rapid peri-probe changes in force control.

From the LC-NE Adaptive Gain perspective, the exploratory finding that participants with lower LC structural integrity showed a non-significant trend toward larger load-induced tremor reactivity is intriguing, but the confidence interval crossed zero and the sample supporting this analysis was small. Until replicated in a larger sample with confirmed LC measurement validity, this association should be treated as a directional hypothesis for future confirmatory work rather than as evidence for a neuromodulatory boundary condition.
...
Despite these limitations, the study makes a methodological contribution by introducing and validating a full trial-level spectral pipeline for continuous grip-force analysis in an aging cognitive-motor paradigm. The pipeline—target normalization, detrending, Butterworth high-pass filtering, Welch PSD, band integration, and mixed-effects logistic regression with subject-wise cross-validation—is reproducible and generalizable to other populations, force levels, and perceptual tasks. The null results here provide empirical benchmarks: under the present conditions, band-averaged spectral power in the 0.5–3 Hz and 8–12 Hz bands carries at most weak trial-level information about perceptual success. Future confirmatory work should test whether probe-locked or post-probe force dynamics, higher-resolution time-frequency features, or participant subgroups with larger load costs show stronger spectral-perceptual coupling.
